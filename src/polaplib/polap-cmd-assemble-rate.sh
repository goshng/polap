...
function _run_polap_assemble-rate {
	...
	help_message=$(
		cat <<'EOF'
Name:
  polap assemble-rate - find a data sampling rate for a good sampling rate of mapped reads

Synopsis:
  polap assemble-rate [options]
...
EOF
	...
	# if [[ ! -s "${_polap_var_oga_contig}"/contig.tab ]]; then
	_run_polap_map-reads
	# fi

	# test-reads and select-reads
	# _run_polap_select-reads
	# _run_polap_test-reads
	#
	# We need a separate lib:
