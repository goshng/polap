...
!include figures/table-benchmark-data-test-0.md
...
<!-- !include figures/table-benchmark-time-test-2.md -->
...
<!-- !include figures/table-benchmark-polap-test-4.md -->
...
Replace, if necessary, the version number 0.5.3.1 with something recommended at [Polap's github website](http://github.com/goshng/polap).
...
wget -q https://github.com/goshng/polap/archive/refs/tags/0.5.3.1.zip
unzip -o -q 0.5.3.1.zip
mv polap-0.5.3.1 polap
# Now, we have the polap folder.
bash polap/src/polap-data-read -y install conda
```
...
bash polap/src/polap-data-read setup conda
```

**3. Install Bioconda packages**:
Install conda packages including `polap` and others in their own conda environments.
We execute `polap-data-read -y install polap` if we want the latest version of `polap`.

```bash
# option 1: to install all tools necessary
bash polap/src/polap-data-read -y install all=0.5.3
# option 2: to install polap only
bash polap/src/polap-data-read -y install polap=0.5.3
bash polap/src/polap-data-read setup polap
bash polap/src/polap-data-read setup pmat
```
...
polap-data-read -y download test-data-read
polap-data-read benchmark Test_species
polap-data-read benchmark Taxon_genus
for i in 3 11 12 21 22; do
  polap-data-read run polap-disassemble Taxon_genus $i
done
...
```bash
polap-data-read -y install bandage
polap-data-read setup bandage
polap-data-read install latex
polap-data-read man init
for i in 0 1 2 4; do polap-data-read man table-benchmark test $i; done
for i in {0..4}; do
  polap-data-read man table-polap-disassemble Taxon_genus $i
done
polap-data-read man figure-sheet test 2 bandage # Requires latex
polap-data-read man figure-sheet-pmat test # Requires latex
polap-data-read man figure-sheet-oatk test # Requires latex
polap-data-read man figure-sheet-tippo test # Requires latex
polap-data-read man figure-benchmark test 2 time
polap-data-read man figure-benchmark test 2 memory
polap-data-read man figure-benchmark test 2 time-nextdenovo
polap-data-read man figure-alpha Taxon_genus
polap-data-read man figure-delta Taxon_genus
polap-data-read man pdf test # Requires latex
```
...
```bash
polap-data-read man table-benchmark test 2 computer view
polap-data-read man table-benchmark test 2 time view
polap-data-read man table-benchmark test 2 memory view
polap-data-read man table-polap-disassemble Taxon_genus 2 view
```
...
polap-data-read benchmark some
# option 2: use a species folder name for a particular dataset
polap-data-read benchmark Eucalyptus_pauciflora
polap-data-read benchmark Spirodela_polyrhiza
for i in 3 {11..29}; do
  polap-data-read run polap-disassemble Eucalyptus_pauciflora $i
done
...
unzip 28740323.zip
for i in *-a.tar.gz; do polap-data-read recover ${i%-a.tar.gz}; done
```
...
```bash
polap-data-read -y man init
for i in 0 1 2 4; do polap-data-read man table-benchmark some $i; done
for i in {0..4}; do
  polap-data-read man table-polap-disassemble Eucalyptus_pauciflora $i
done
polap-data-read man figure-sheet some 2 bandage # Requires latex
polap-data-read man figure-sheet-pmat some # Requires latex
polap-data-read man figure-sheet-oatk some # Requires latex
polap-data-read man figure-sheet-tippo some # Requires latex
polap-data-read man figure-benchmark some 2 time
polap-data-read man figure-benchmark some 2 memory
polap-data-read man figure-benchmark some 2 time-nextdenovo
polap-data-read man figure-alpha Eucalyptus_pauciflora
polap-data-read man figure-delta Eucalyptus_pauciflora
polap-data-read man pdf # Requires latex
```
...
```bash
polap-data-read man table-benchmark some 2 computer view # Table S2
polap-data-read man table-benchmark some 2 data view # Table S1
polap-data-read man table-benchmark some 2 time view # Table for Figure 2 and S3
polap-data-read man table-benchmark some 2 memory view # Table for Figure 3
polap-data-read man table-benchmark some 2 polap view # Table 1
polap-data-read man table-benchmark some 1 polap view # Table S3
polap-data-read man table-benchmark some 0 polap view # Table S4
polap-data-read man table-benchmark some 4 polap view # Table S5
polap-data-read man table-polap-disassemble Eucalyptus_pauciflora 2 view # Table 2
polap-data-read man table-polap-disassemble Eucalyptus_pauciflora 0 view # Table S6
polap-data-read man table-polap-disassemble Eucalyptus_pauciflora 3 view # Table S7
polap-data-read man figure-benchmark some 2 time view # Figure 2
polap-data-read man figure-benchmark some 2 memory view # Figure 3
polap-data-read man figure-alpha Eucalyptus_pauciflora view # Figure S1
polap-data-read man figure-delta Eucalyptus_pauciflora view # Figure S2
polap-data-read man figure-benchmark some 2 time-nextdenovo view # Figure S3
polap-data-read man figure-sheet some 2 no-bandage # Genome assembly graphs
```

<!-- benchmark assembly graph figure -->
<!-- polap-data-read man figure-sheet some 2 -->
