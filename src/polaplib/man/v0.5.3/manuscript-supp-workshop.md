...
bash polap/src/bolap.sh -y install conda
```
...
bash polap/src/bolap -y install all=0.4.3
...
```bash
cd
cp -pr /media/h1/share/5_H_hon 5_H_hon
cd 5_H_hon
mv o o2
git clone https://github.com/goshng/polap.git
cd polap
...
cd -
source ~/miniconda3/bin/activate
mv 11_all.fq l.fastq
gunzip H-hon.R1.fq.gz
gunzip H-hon.R2.fq.gz
mv H-hon.R1.fq s_1.fastq
mv H-hon.R2.fq s_2.fastq
ls

bash polap/src/bolap config add Hemerocallis_hongdoensis l s
bash polap/src/bolap benchmark Hemerocallis_hongdoensis

conda activate polap
mv o o3
cp -pr Hemerocallis_hongdoensis/v5/0/polap-assemble-wga o
polap annotate view
