...
# We should have one for all because they are very similar to one another.
# It is one of the most important R script in polap.
#
#
...
parser <- add_option(parser, c("--use-strand"),
  action = "store_true",
  default = FALSE,
  help = "Use only the plus strand mapped reads; this will create ptgaul.names"
)
...
# for pair-mapping we need the seed contig names
# mt.contig.name-x
if (!is.null(args1$mtcontigname)) {
  x1 <- as_tibble(read.table(args1$mtcontigname))
} else {
  x1 <- tibble() # safe for nrow(x1), ncol(x1), dplyr pipes, etc.
}
