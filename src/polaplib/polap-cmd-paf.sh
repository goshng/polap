...
EOF
	...
	Rscript --vanilla "${_POLAPLIB_DIR}/polap-r-paf-qc-suggest.R" \
		-i "${_arg_infile}" \
		-o "${_arg_outdir}"

	# parse suggestions back into your reducer call
	eval "$(
		Rscript --vanilla "${_POLAPLIB_DIR}/polap-r-paf-qc-suggest.R" \
			-i "${_arg_infile}" \
			-o /dev/null
	)"
	_polap_log0 "--min_olen $min_olen --min_ident $min_ident --w_floor $w_floor"
