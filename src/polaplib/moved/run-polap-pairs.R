...
#
# NOTE: move it because we do not use it. We should delete the following:
# function _run_polap_x-v0.2.6-select-reads
################################################################################
...
if (nrow(x) > 1) {
  ...
  orders <- function(y) {
    ...
    y1 <- x %>%
      filter(V6 == y[1], V10 / V11 > 0.7, V11 > pair_min, V7 > brigde_min) %>%
      select(V1) %>%
      distinct(V1)
    y2 <- x %>%
      filter(V6 == y[2], V10 / V11 > 0.7, V11 > pair_min, V7 > brigde_min) %>%
      select(V1) %>%
      distinct(V1)
  ...
  x %>%
    filter((V4 - V3) / V2 > 0.7, V10 / V11 > 0.7, V11 > single_min) %>%
    select(V1) %>%
    distinct(V1) %>%
    write.table(sfile, row.names = F, col.names = F, quote = F)
  ...
  # x %>% filter((V4-V3)/V2 > 0.7, V10/V11 > 0.7) %>% select(V1) %>% distinct(V1) %>% write.table(sfile,row.names=F,col.names=F,quote=F)
  x %>%
    filter(V10 / V11 > 0.7, V11 > single_min) %>%
    select(V1) %>%
    distinct(V1) %>%
    write.table(sfile, row.names = F, col.names = F, quote = F)
}
