...
# 2025-06-16
# SCC:
# This is a version of ptGAUL with the genome size option included.
# The orginal -g option was not properly used in the execution of flye.
# There are two lines that were edited for that.
# Before:
# flye --nano-raw $OPD/result_$FRL/filter_reads_final.fa --genome-size 0.16m \
# After:
# flye --nano-raw $OPD/result_$FRL/filter_reads_final.fa --genome-size $GEM \
################################################################################
...
if [ ! -s $OPD/result_$FRL/total_length_of_plastid_reads ]; then
	...
	if [[ $num = "" ]]; then
		...
		flye --nano-raw $OPD/result_$FRL/new_filter_gt$FRL.fa --genome-size $GEM \
			--out-dir $OPD/result_$FRL/flye_cpONT --threads $NUM_THREADS --asm-coverage $asm_cov 2>>error.txt
...
if [[ $number_edge -eq 3 ]]; then
	...
	# END
	echo "python $SCRIPT_DIR/combine_gfa.py -e $OPD/result_$FRL/edges.fa -d $OPD/result_$FRL/sorted_depth -o $OPD/result_$FRL/ptGAUL_final_assembly/ 2>>error.txt"
	end=$(date +%s)
