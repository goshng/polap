...
function _polap_disassemble-cflye {
  ...
  if [[ "${CONDA_DEFAULT_ENV:-}" != "polap-cflye" ]]; then
    if [[ "$DEBUG" == "1" ]]; then
      echo "[INFO] Activating conda environment 'polap-cflye'..."
    fi
    conda activate polap-cflye
  fi

  # Check the activation of the polap-cflye conda environment
  if [[ "${CONDA_DEFAULT_ENV:-}" != "polap-cflye" ]]; then
    echo "[ERROR] Failed to enter conda environment 'polap-cflye'"
    return
  fi
