...
# polaplib/run-polap-r-determine-depth-range-4.R
# Check: 2025-06-17
...
#
# TODO: rename: polap-r-determine-depth-range_4.R
#
# See Also:
...
if (is_null(args1$table)) {
  ...
  input1 <- file.path(input_dir0, "assembly_info_organelle_annotation_count-all_Anthoceros_agrestis.txt")
...
if (nrow(xt) > 1) {
  ...
  xt_summary |> write_tsv(args1$out)
} else if (nrow(xt) == 1) {
  xt_summary <- xt |>
    summarise(
