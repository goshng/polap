...
################################################################################
# Functions for subcommands to infer phylogenies using mtDNA or ptDNA genomes.
# This is a part of the polap-data-taxon script.
# It needs further development in the preparation of genes extracted from
# organellel genomes.
#
...
function _run_polap_taxonomy-sample {
	...
	if _polap_contains_step 11 "${_step_array[@]}"; then
		...
		_polap_log3_pipe "Rscript ${_POLAPLIB_DIR}/run-polap-r-taxonomy.R \
			sample \
...
function _run_polap_taxonomy-orthofinder {
	...
	if _polap_contains_step 5 "${_step_array[@]}"; then
		...
		Rscript --vanilla "${_POLAPLIB_DIR}/run-polap-r-orthofinder.R" \
