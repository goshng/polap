...
#
################################################################################
function _run_polap_directional-test-reads { # selects reads mapped on a genome assembly
  # Enable debugging if DEBUG is set
  [ "$DEBUG" -eq 1 ] && set -x
  _polap_log_function "Function start: $(echo $FUNCNAME | sed s/_run_polap_//)"
...
HEREDOC
  ...
  if [[ "${_arg_menu[1]}" == "view" ]]; then
    ...
    run_polap_directional-test-reads
...
function _polap_directional-dflye {
  ...
  if [[ "${_POLAP_RELEASE}" -eq 0 ]]; then
    _command1="command time -v /home/goshng/all/polap/Flye/bin/dflye"
  else
...
HEREDOC
  ...
  _polap_log3_cmd bash "$script_dir/run-polap-sh-minimap2-paf2tab.sh" "${_arg_min_read_length}" "${_directional_dir}/01-contig-contig1.paf" "${_directional_dir}/01-contig-contig1.tab"
  ...
  _polap_log3_cmd bash "$script_dir/run-polap-sh-minimap2-paf2tab.sh" "${_arg_min_read_length}" "${_directional_dir}/01-contig-contig2.paf" "${_directional_dir}/01-contig-contig2.tab"
