...
HEREDOC
	...
	_polap_log0 "mapping long-reads on the seed contigs ..."
	...
	_polap_log2 "    input2: ${_polap_var_base_lk_fq_gz}"
...
#   ${_polap_var_oga_reads}/contig.tab
################################################################################
function _run_polap_collect-reads() { # selects reads mapped on a genome assembly
	# Enable debugging if DEBUG is set
	[ "$DEBUG" -eq 1 ] && set -x
	_polap_log_function "Function start: $(echo $FUNCNAME | sed s/_run_polap_//)"
...
HEREDOC
	...
	_polap_opt_dict["ptgaul-best"]="ptgaul"
	_polap_opt_dict["single-best"]="single"
	_polap_opt_dict["polap-best"]="combined"
	...
	if [[ "${_arg_long_reads_is}" == "off" ]]; then
		local _source_long_reads_fq="${_polap_var_base_lk_fq_gz}"
		_polap_log2 "    we utilize all available size-limited long-read data for our analysis."
	...
	*)
	...
	for ((i = ${_arg_start_index}; i < array_length; i++)); do
		...
		*-intra-read-ratio)
			die "Not implemented yet!"
			;;
		*-intra-base-length | ptgaul-best | single-best)
		...
		combined-bridge-read-length)
			...
			;;
		polap-rw-base-length | polap-best)
			_arg_single_min="${restored_array[i]}"
			_arg_pair_min="${restored_array[i]}"
			_arg_bridge_min="0"
			;;
      ...
      --all \
		...
		if [[ -d "${_polap_var_base_lk_fq_gz}.split" ]]; then
			ls "${_polap_var_base_lk_fq_gz}.split"/* |
				parallel seqtk subseq {} "${_polap_var_oga_reads}/${_pread_sel}/${i}/${_read_names}.names" ">" {}.fq
		...
		if [[ "$_expected_organelle_coverage" -gt "${_arg_coverage}" ]] &&
			[[ "${_arg_coverage_check}" == "on" ]]; then
			...
			_polap_log1 "    random seed for reducing the single-mapped long-read data: ${_random_seed}"
			_polap_log3_pipe "seqkit sample \
        -p ${_rate} \
        -s ${_random_seed} \
			  ${_polap_var_oga_seeds}/${_pread_sel}/${i}.fq.gz \
      ...
      --nano-raw ${_polap_var_oga_sample}/${_pread_sel}/${i}.fq.gz \
        ...
        >${_polap_var_oga_summary}/${_pread_sel}/${i}.bases"
			...
			_polap_log0 "No such graph: ${_polap_var_oga_flye}/${_pread_sel}/${i}/30-contigger/graph_final.gfa"
		fi
	done
...
#
################################################################################
function _run_polap_best-reads() { # selects reads mapped on a genome assembly
	# Enable debugging if DEBUG is set
	[ "$DEBUG" -eq 1 ] && set -x
	_polap_log_function "Function start: $(echo $FUNCNAME | sed s/_run_polap_//)"
...
# Menus:
#
Example: $0 ${_arg_menu[0]} [ptgaul-best] -w 3000
Example: $0 ${_arg_menu[0]} single-best -w 3000
Example: $0 ${_arg_menu[0]} polap-best -w 3000
HEREDOC
	)
	...
	if [[ "${_arg_menu[1]}" == "infile" ]]; then
		_arg_menu[1]="ptgaul-best"
		_polap_log0 "  default set to read-selection: ${_arg_menu[1]}"
	fi
	_arg_select_read_range="${_arg_single_min},${_arg_single_min},1"
	_arg_flye="off"
...
# Best flye2
################################################################################
function _run_polap_best-flye2() { # executes Flye for an organelle-genome assembly
	# Enable debugging if DEBUG is set
	[ "$DEBUG" -eq 1 ] && set -x
	_polap_log_function "Function start: $(echo $FUNCNAME | sed s/_run_polap_//)"
...
Example: $0 ${_arg_menu[0]} [ptgaul-best] -j <arg>
Example: $0 ${_arg_menu[0]} single-best -j <arg>
Example: $0 ${_arg_menu[0]} combined-best -j <arg>
HEREDOC
	)
	...
	_polap_log1 "    output: ${_polap_var_oga}/30-contigger/graph_final.gfa"
	local _command1="flye \
