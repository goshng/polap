...
function _disassemble-hifi-stage0 {
	...
	# skip the short-read downsampling for pacbio-hifi data.
	if [[ "${_arg_data_type}" == "pacbio-hifi" ]]; then
		return
	fi
...
HEREDOC
	...
	# no polishing for pacbio-hifi data
	if [[ "${_arg_data_type}" == "pacbio-hifi" ]]; then
		_arg_stages_exclude="3"
	fi
...
HEREDOC
	...
	cat <"${_index_table}" | while IFS=$'\t' read -r i sampling_datasize genomesize; do
		...
		if _polap_contains_step 7 "${_step_array[@]}"; then
			_polap_log1 "  step 7: genome size using the subsample of the read data"
			if [[ "${_arg_data_type}" == "pacbio-hifi" ]]; then
				check_file_existence "${_long_read}"
				_polap_log2 "    input1: ${_long_read}"
