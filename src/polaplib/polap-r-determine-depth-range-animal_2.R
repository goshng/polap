...
if (is_null(args1$table)) {
  ...
  input1 <- file.path(input_dir0, "animal-assembly_info_organelle_annotation_count-all.txt")
  output1 <- file.path(input_dir1, "animal-custom.depth.range-1.txt")
  output2 <- file.path(input_dir1, "animal-contig-annotation-cdf-table.txt")
...
cutoff_data <- x0 |>
  ...
  filter(!(MT >= PT & dispersion_MT > 1e+4)) |>
