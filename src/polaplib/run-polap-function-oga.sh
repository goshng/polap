...
HEREDOC
	...
	if [[ ${_arg_circularize} == "on" ]]; then
		if [ "$contig_count" -eq 1 ]; then
			...
			_polap_log3_cmd bash "${_POLAPLIB_DIR}/run-polap-sh-half-cut.sh" "${MTDIR}" "${MTDIR}/contig.fa" "${_polap_var_mtcontigname}"
	...
	_polap_log3_cmd bash "${_POLAPLIB_DIR}/run-polap-sh-minimap2-paf2tab.sh" "${_arg_min_read_length}" "${MTDIR}/contig.paf" "${MTDIR}/contig.tab"
