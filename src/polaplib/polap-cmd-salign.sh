...
EOF
	...
	if [[ "${_arg_menu[1]}" == "check" ]]; then
		_polap_lib_salign-check-pt "${_arg_menu[2]}" \
			--out "${_arg_outdir}/salign/pt" \
			--tmp-in-out
		return 0
	fi

	# align
	# bash "${_POLAPLIB_DIR}/polap-bash-align-ptdna.sh" "${polap_ptdna_gfa}" "${tippo_ptdna_gfa}" --out "${mafftdir}/ptgaul-tippo"
	_polap_lib_salign-pt "${_arg_menu[1]}" "${_arg_menu[2]}" \
		--out "${_arg_outdir}/salign/pt" \
		--tmp-in-out
