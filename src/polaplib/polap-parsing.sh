...
parse_commandline() {
	...
	while test $# -gt 0; do
		...
		--no-reduction-reads | --reduction-reads)
			...
			;;
		--no-readassemble-pt | --readassemble-pt)
			_arg_readassemble_pt="on"
			test "${1:0:5}" = "--no-" && _arg_readassemble_pt="off"
			;;
		...
		--sim-nuc=*)
			...
			;;
		--no-use-oatk | --use-oatk)
			_arg_use_oatk="true"
			test "${1:0:5}" = "--no-" && _arg_yes="false"
			;;
		--version)
			_polap_lib_version
...
_arg_verbose_str=$(verbose_or_quiet_flags)
