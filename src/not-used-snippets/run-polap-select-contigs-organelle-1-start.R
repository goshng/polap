...
if (length(args) > 0) {
  ...
  input_dir0 <- paste0("/media/h2/goshng/figshare/", s, "/o/1/2/mtcontigs")
...
# y <- x1 |> filter(Copy < 5 * median(x1$Copy), Copy > 0.2 * median(x1$Copy))

# FIXME: multiple edegs per contig
x1 |> mutate(edgename=paste0("edge_",Edge)) |> select(edgename, MT) |>
