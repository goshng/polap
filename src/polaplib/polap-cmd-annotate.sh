...
HEREDOC
	...
	if [[ "${_arg_menu[1]}" == "seed" ]]; then
		_polap_log0 "create seed: ${_polap_var_mtcontigname}"
		case "${_arg_menu[2]}" in
		all)
			awk 'NR > 1 { print $1 }' "${_polap_var_ga_annotation_all}" \
				>"${_polap_var_mtcontigname}"
			;;
		pt-table | pt)
			awk 'NR > 1 { print $1 }' "${_polap_var_ga_pt_annotation_depth_table}" \
				>"${_polap_var_mtcontigname}"
			;;
		mt | table | *)
			awk 'NR > 1 { print $1 }' "${_polap_var_ga_annotation_depth_table}" \
				>"${_polap_var_mtcontigname}"
			;;
		esac
...
HEREDOC
	...
	_polap_log1 NEXT: $(basename "$0") seeds -o "${_arg_outdir}" -i ${_arg_inum} -j ${_arg_jnum}
...
# 2025-07-07
# removed for-loop log
#
# 2025-07-19
# add --nucleotide option
polap_blast-genome() { # BLAST edge sequences on MT and PT genes
	...
	local BLAST_CMD="tblastn"
	if [[ "${_arg_nucleotide}" == "on" ]]; then
		MTAA="${_POLAPLIB_DIR}"/polap-mt.1.c70.3.fna
		PTAA="${_POLAPLIB_DIR}"/polap-pt.2.c70.3.fna
		BLAST_CMD="blastn"
	fi
	...
	_polap_log2 "    evalue cutoff: 1e-${_arg_blast_evalue}"
	_polap_log2 "    number of CPUs: ${_arg_threads}"
	_polap_log2 "    executing the ${BLAST_CMD} ... be patient!"
	_polap_log3_pipe "${BLAST_CMD} -query ${MTAA} \
		-db ${_ann_CONTIGDB} \
		-out ${_ann_MTAABLAST} \
		-evalue 1e-${_arg_blast_evalue} \
	...
	## version 2
	# while IFS= read -r contig; do
	# 	grep -w ${contig} ${_ann_MTAABLAST}.sorted.bed \
	# 		>${_ann_MTAABED}/${contig}.bed
	# 	bedtools merge -i ${_ann_MTAABED}/${contig}.bed \
	# 		>${_ann_MTAABED}/${contig}.bed.txt
	# 	printf "%s\t%d\n" ${contig} $(wc -l <${_ann_MTAABED}/${contig}.bed.txt) >>"${_ann_MTGENECOUNT_TMP}"
	# done <"${_ann_CONTIGNAME}"

	## version 3
	export _ann_MTAABLAST _ann_MTAABED

	mkdir -p "${_ann_MTAABED}" # make sure output folder exists

	parallel --jobs 16 --linebuffer --halt soon,fail=1 '
    contig={}

    bed="${_ann_MTAABED}/${contig}.bed"
    bedtxt="${_ann_MTAABED}/${contig}.bed.txt"
    out="${_ann_MTAABED}/${contig}.out"

    # Run grep (may produce empty file)
    grep -w "$contig" "${_ann_MTAABLAST}.sorted.bed" > "$bed" || true

    # Run bedtools only if non-empty
    if [[ -s "$bed" ]]; then
        bedtools merge -i "$bed" > "$bedtxt"
    else
        touch "$bedtxt"
    fi

    # Output contig + line count
    printf "%s\t%d\n" "$contig" "$(grep -c "^" "$bedtxt")" > "$out"
' :::: "${_ann_CONTIGNAME}"

	# Combine output
	# cat "${_ann_MTAABED}"/*.out >"${_ann_MTGENECOUNT_TMP}"
	find "${_ann_MTAABED}" -name '*.out' -print0 | xargs -0 cat >"${_ann_MTGENECOUNT_TMP}"
	...
	_polap_log2 "    number of CPUs: ${_arg_threads}"
	_polap_log2 "    executing the ${BLAST_CMD} ... be patient!"
	_polap_log3_pipe "${BLAST_CMD} -query ${PTAA} \
		-db ${_ann_CONTIGDB} \
		-out ${_ann_PTAABLAST} \
		-evalue 1e-${_arg_blast_evalue} \
		-outfmt '6 qaccver saccver pident length mismatch gapopen qstart qend sstart send evalue bitscore stitle salltitles' \
	...
	## version 2
	# while IFS= read -r contig; do
	# 	grep -w ${contig} ${_ann_PTAABLAST}.sorted.bed \
	# 		>${_ann_PTAABED}/${contig}.bed
	# 	bedtools merge -i ${_ann_PTAABED}/${contig}.bed \
	# 		>${_ann_PTAABED}/${contig}.bed.txt
	# 	printf "%s\t%d\n" ${contig} $(wc -l <${_ann_PTAABED}/${contig}.bed.txt) >>"${_ann_PTGENECOUNT_TMP}"
	# done <"${_ann_CONTIGNAME}"

	## version 3
	export _ann_PTAABLAST _ann_PTAABED

	mkdir -p "${_ann_PTAABED}" # make sure output folder exists

	parallel --jobs 16 --linebuffer --halt soon,fail=1 '
    contig={}

    bed="${_ann_PTAABED}/${contig}.bed"
    bedtxt="${_ann_PTAABED}/${contig}.bed.txt"
    out="${_ann_PTAABED}/${contig}.out"

    # Run grep (may produce empty file)
    grep -w "$contig" "${_ann_PTAABLAST}.sorted.bed" > "$bed" || true
...
' :::: "${_ann_CONTIGNAME}"
	...
	# Combine output
	# cat "${_ann_PTAABED}"/*.out >"${_ann_PTGENECOUNT_TMP}"
	find "${_ann_PTAABED}" -name '*.out' -print0 | xargs -0 cat >"${_ann_PTGENECOUNT_TMP}"
