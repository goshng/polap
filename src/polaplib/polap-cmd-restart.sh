...
Examples:
  Get organelle genome sequences:
    polap ${polap_cmd} --infile pt.1.fasta
...
EOF
	...
	if [[ ! -s "${_arg_outdir}/plastid-anchor-genes/anchors_plastome_universal.fa" ]]; then
		_run_polap_download "anchor"
	fi

	local OUTDIR="${_arg_outdir}/plastome"
	_polap_log3_cmd rm -rf "$OUTDIR"
	_polap_log3_cmd mkdir -p "$OUTDIR"

	_polap_log0_cmd bash "${_POLAPLIB_DIR}/polap-bash-standardize-plastome.sh" \
		-i "${_arg_infile}" \
		-a "${_arg_outdir}/plastid-anchor-genes/anchors_plastome_universal.fa" \
		-o "${OUTDIR}" \
		--prefer-plus

	_polap_log0 "File: $OUTDIR"
