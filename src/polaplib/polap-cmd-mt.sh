...
function _run_polap_mt {
	...
	help_message=$(
		cat <<'EOF'
Name:
  polap mt - extract mtdna fram a gfa

Synopsis:
  polap mt [options]

Description:
  polap mt extracts mtDNA sequences from a gfa.
...
EOF
	...
	# read gfa
	_polap_lib_mt-extract-dna "${_arg_infile}" "${_arg_outdir}"
