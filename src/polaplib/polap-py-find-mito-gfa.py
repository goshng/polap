...
def walk_maximal_path(graph, segments, start_node):
    """
    Greedy walk through component from the starting edge orientation
    """
    visited = set()
    path = []
    current = start_node
    ...
    while True:
        node = current
        edge_name, orient = node[:-1], node[-1]
        if edge_name in visited:
            break
        path.append((edge_name, orient))
        visited.add(edge_name)

        neighbors = list(graph.neighbors(node))
        # Prefer unvisited
        next_nodes = [n for n in neighbors if n[:-1] not in visited]
        if not next_nodes:
            break
        current = next_nodes[0]  # pick one arbitrarily
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract plant mitochondrial genome component from GFA."
    )
    parser.add_argument("gfa", help="Input GFA file")
    parser.add_argument(
        "--seed", default="edge_1", help="Comma-separated list of seed edges"
    )
    parser.add_argument(
        "--mtcontig",
        default="mt.contig.name-1",
        help="Output file for mitochondrial edge list",
    )
    parser.add_argument(
        "--fasta",
        default="-",
        help="Output FASTA file for mitochondrial sequence (default: stdout)",
    )
    ...
    # Handle multiple seeds (e.g., edge_1,edge_5)
    candidate_seeds = args.seed.split(",")
    candidate_seeds = [s if s[-1] in "+-" else s + "+" for s in candidate_seeds]

    segments, graph, component = None, None, None
    ...
    if not graph:
        print("No connected component found for given seeds.")
        return

    print(f"Using seed: {seed}")

    # Walk through graph to find a spanning path
    path = walk_maximal_path(graph, segments, seed)

    print_component_edges_by_role(path)

    # if args.mtcontig:
    #     write_edge_list(path, args.mtcontig)
    #     print(f"Wrote mitochondrial edge list to {args.mtcontig}")

    if args.mtcontig:
        # Extract all unique segment names from the graph
        all_edges = sorted({node[:-1] for node in graph.nodes})
        with open(args.mtcontig, "w") as f:
    ...
    if args.fasta:
        ...
        write_fasta_to_stream(seq, args.fasta, seq_id="mitochondrion")
        if args.fasta != "-":
            print(f"Wrote mitochondrial FASTA to {args.fasta}")
