...
read-a-tsv-file-into-associative-arrays() {
	...
	while IFS=$',' read -r species long short random_seed downsample dummy status; do
		# Skip header line
...
polap-clean_genus_species() {
	...
	for i in assemble1 seeds assemble2; do
		local _outdir="${_brg_outdir}"-"${_brg_inum}"-polap-"${i}"
		rm -rf "${_outdir}"
	done
}
...
maintable1_genus_species_for() {
	...
	if [[ ! -s "${_brg_outdir_i}/polap-assemble2.log" ]]; then
		echo "No result yet: ${_brg_outdir_i}" >&2
		return
	fi
	...
	local l_fq_stats="${_brg_outdir}/${_brg_inum}/l.fq.stats"
	local _l_sra=$(extract_long_sra_seqkit_stats "${l_fq_stats}")
	# Extract long-read SRA data size in GB
	local _l_sra_size_gb=$(extract_long_sra_size_gb "${_brg_outdir_i}")

	# Extract short-read SRA ID
	local s_fq_stats="${_brg_outdir}/${_brg_inum}/s1.fq.stats"
	local _s_sra=$(extract_long_sra_seqkit_stats "${s_fq_stats}")

	# Extract short-read SRA data size in GB
	local _s_sra_size_gb=$(extract_short_sra_size_gb "${_brg_outdir_i}")
	local _s_sra_size=$(extract_short_sra_size "${_brg_outdir_i}")

	# Known ptDNA NCBI accession
	# local _known_mtdna=$(extract_known_mtdna_accession "${_polap_log}")

	# Extract the genome size estimate
	local _genome_size=$(extract_genome_size "${_brg_outdir_i}")

	local _memory_gb='NA'
	local _total_hours='NA'
	extract_time_memory_timing "${_brg_outdir_i}/timing-polap-assemble1.txt"
	local _memory_gb_polap_assemble1=$_memory_gb
	local _total_hours_polap_assemble1=$_total_hours

	extract_time_memory_timing "${_brg_outdir_i}/timing-polap-assemble2.txt"
	local _memory_gb_polap_assemble2=$_memory_gb
	local _total_hours_polap_assemble2=$_total_hours

	extract_time_memory_timing "${_brg_outdir_i}/timing-polap-polish.txt"
	local _memory_gb_polap_polish=$_memory_gb
	local _total_hours_polap_polish=$_total_hours

	extract_time_memory_timing "${_brg_outdir_i}/timing-polap-seeds.txt"
	local _memory_gb_polap_seeds=$_memory_gb
	local _total_hours_polap_seeds=$_total_hours

	extract_time_memory_timing "${_brg_outdir}/0/timing-polap-msbwt-k-x.txt"
	local _memory_gb_polap_msbwt=$_memory_gb
	local _total_hours_polap_msbwt=$_total_hours

	# sequencing data coverage
	local _genome_size=$(<"${_brg_outdir_i}/short_expected_genome_size.txt")
	local _long_total_length=$(<"${_brg_outdir_i}/long_total_length.txt")
	local _long_coverage=$(echo "scale=2; ${_long_total_length} / ${_genome_size}" | bc)
	...
	if ((_brg_table == 1)); then
		_items=(
			...
			"${_memory_gb_polap_seeds}"
			"${_total_hours_polap_seeds}"
		)
...
maintable1_genus_species() {
	...
	if [[ "${_brg_outdir}" == "all" ]]; then
		for outdir in "${S[@]}"; do
			maintable1_genus_species_for "${outdir}" \
				"${_brg_inum}" "${_brg_table}" "${_brg_t_dir}" \
				>>"${_table_tsv}"
		done
	else
	...
	if ((_brg_table == 1)); then
		csvtk -t cut -f Species,Order,Family,D,L_cov,S_cov,Ma1,Ta1,Ma2,Ta2,Mp1,Tp1,Mp2,Tp2,Ms,Ts \
			${_table_tsv} |
			csvtk -t rename -f 1-16 -n Species,Order,Family,D,L_cov,S_cov,Ma1,Ta1,Ma2,Ta2,Mp1,Tp1,Mp2,Tp2,Ms,Ts |
			csvtk -t csv2md -a right -o ${_table_name}-${_brg_inum}-analysis.md
		echo ${_table_name}-${_brg_inum}-analysis.md
	elif ((_brg_table == 2)); then
...
maintable1)
	if [[ "${_arg2}" == arg2 ]]; then
		echo "Help: ${subcmd1} <all|outdir> <inum:0|N> <table:1|N> <targe dir>"
		echo "  $0 $subcmd1 all"
