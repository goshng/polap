...
run_flye2() {
	...
	"$WDIR"/run-polap-pairs.R $ADIR/mt.contig.name-$STEP4 $MTODIR/contig.tab $MTSEEDSDIR $PAIR_MIN $SINGLE_MIN >/dev/null 2>&1
#	"$WDIR"/run-polap-pairs3.R $ADIR/mt.contig.name-$STEP4 $MTODIR/contig.tab $MTSEEDSDIR $PAIR_MIN >/dev/null 2>&1
	echo "OPTION polap pairs: $PAIR_MIN and $SINGLE_MIN"
...
function assemble2() {
	MR=3000
	while getopts "i:o:t:m:a:c:u" option; do
		case $option in
		i) STEP4=$OPTARG ;;
		o) ODIR=$OPTARG ;;
		t) NT=$OPTARG ;;
