...
for ((i = 4; i < $N; i++)); do
	...
	elif [ -s of/$A.faa ]; then
		seqkit seq -n of/$A.faa | cut -d' ' -f1-2 >2
	fi
	...
	"$MDIR"/run-polap-confirm-genes.R 1 2 $A of/genes/$j >/dev/null 2>&1
...
"$MDIR"/run-polap-confirm-genes2.R of/genes.csv of/genes.target.txt phylogenies/genes.tsv >/dev/null 2>&1
