...
HEREDOC
	...
	# NOTE: we have no short-read stat step.
	#
...
Example:
$(basename "$0") ${_arg_menu[0]} -a <file> [-b <file>]
$(basename "$0") ${_arg_menu[0]} -o ${_arg_outdir} --bioproject use
HEREDOC
	)
...
# Pacbio data genome size
function _run_polap_find-genome-size-for-pacbio { # estimate the whole genome size
	# Enable debugging if _POLAP_DEBUG is set
	[ "$_POLAP_DEBUG" -eq 1 ] && set -x
	_polap_log_function "Function start: $(echo $FUNCNAME | sed s/_run_polap_//)"
...
HEREDOC
	...
	_polap_log0 "estimate the genome size using your long-read data [${_arg_long_reads}]"
	...
	if [ -s "${_polap_var_outdir_genome_size}" ] && [ "${_arg_redo}" = "off" ]; then
		...
		if [ -s "${_polap_var_outdir_jellyfish_out}" ] && [ "${_arg_redo}" = "off" ]; then
          ...
          -C -m 21 \
				...
				die "ASSERT: we must have a long-read fastq file."
			fi
			check_file_existence "${_polap_var_outdir_jellyfish_out}"
		fi
		...
		if [ -s "${_polap_var_outdir_jellyfish_out_histo}" ] && [ "${_arg_redo}" = "off" ]; then
			_polap_log1 "  found: ${_polap_var_outdir_jellyfish_out_histo}, so skipping the JellyFish hist step ..."
			_polap_log1_file "${_polap_var_outdir_jellyfish_out_histo}"
		else
			_polap_log3_pipe "jellyfish histo \
		  	-t ${_arg_threads} \
			  -o ${_polap_var_outdir_jellyfish_out_histo} \
...
HEREDOC
	...
	if [ -s "${_polap_var_wga_contigger_edges_gfa}" ] &&
		...
		if [[ "${_arg_blast}" == "off" ]]; then
			...
			if [[ "${_arg_timing}" == "off" ]]; then
				# local _command1="$HOME/all/polap/dFlye/bin/bflye"
				local _command1="flye"
			else
				# local _command1="command time -v bflye"
				local _command1="command time -v flye"
			fi
		fi
		_command1+=" \
      ...
      ${_polap_var_outdir_nk_fq_gz} \
		...
		# if [[ "${_arg_blast}" == "on" ]]; then
		# 	_command1+=" \
		#       --disjointig-min-mtdensity ${_arg_blast_mt} \
		#       --disjointig-min-ptdensity ${_arg_blast_pt}"
		# fi

		# flye1 or the whole-genome assembly, wga, polap stage
		# default: stop-after contigger
		#
		# You might also resume from a particular stage with
		# --resume-from stage_name, where stage_name is a choice of
		# assembly, consensus, repeat, contigger, polishing.
		if [[ "${_arg_menu[1]}" == "polishing" ]] ||
			[[ "${_arg_menu[1]}" == "assembly" ]] ||
			[[ "${_arg_menu[1]}" == "consensus" ]] ||
			[[ "${_arg_menu[1]}" == "repeat" ]] ||
			[[ "${_arg_menu[1]}" == "contigger" ]]; then
			_command1+=" \
		  --stop-after ${_arg_menu[1]}"
		elif [[ "${_arg_menu[1]}" == "resume" ]]; then
			_command1+=" \
		  --resume"
		elif [[ "${_arg_menu[1]}" == "resume-from" ]]; then
			if [[ "${_arg_menu[2]}" == "assembly" ]] ||
				[[ "${_arg_menu[2]}" == "consensus" ]] ||
				[[ "${_arg_menu[2]}" == "repeat" ]] ||
				[[ "${_arg_menu[2]}" == "contigger" ]]; then
				_command1+=" \
		  --resume-from ${_arg_menu[2]}"
			fi
		else
			_command1+=" \
	...
	if [[ -s "${_polap_var_wga_contigger_edges_gfa}" ]]; then
		...
		_polap_log0 "ERROR: no such file: ${_polap_var_wga_contigger_edges_gfa}"
		return "${RETURN_FAIL}"
	fi
