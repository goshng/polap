...
Examples:
  Get organelle genome sequences:
    polap ${polap_cmd} -l l.fq --infile polished.fa
...
EOF
	...
	if [[ "${_arg_plastid}" == "on" ]]; then
		OUTDIR="${_arg_outdir}/coverage/pt"
	else
		OUTDIR="${_arg_outdir}/coverage/mt"
	fi
	_polap_log3_cmd rm -rf "$OUTDIR"

	# _polap_log0 bash "${_POLAPLIB_DIR}/polap-bash-organelle-coverage.sh" \
	_polap_log3_cmd bash "${_POLAPLIB_DIR}/polap-bash-organelle-coverage.sh" \
		--assembly "${_arg_infile}" \
		--reads "${_arg_long_reads}" \
		--outdir "${OUTDIR}" \
		--preset map-ont --mq 20 --bin 500 \
		-t 8
